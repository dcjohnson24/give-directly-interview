...
def main(show_plots: bool = False):
    ...
    # It appears that all the recipients have had some survey attempt. We can
    # drop the _merge variable now
    merged.drop(columns='_merge', inplace=True)
    # Check for duplicates. We expect that recipient_id and survey_id together
    # form a unique id
    merged[merged.duplicated(subset=['recipient_id', 'survey_id'], keep=False)]
    # There are duplicates that must be dropped.
    merged = merged.drop_duplicates(
        subset=['recipient_id', 'survey_id'],
        keep='first'
    ...
    # Set the value to start for those with no successful surveys.
    # '~' negates the value of the mask. In this case,
    # ~mask means find those without any successful survey
    ...
    # There are 6 respondents without an account status. We could assign
    # these to not Active but it is important to follow up with a field team
    # for this answer.
    ...
    # 102 successful surveys in December
    ...
    # To deal with these problematic cases, I replace them with missing values
    # to be imputed later.
    ...
    merged.isna().sum()
    # Check with field teams or relevant departments to resolve these issues
    ...
    # We can see there is a correlation between account_number
    # and account_status being missing,
    # This would make sense considering that someone without an account number
    #  wouldn't have a status and vice versa. There is also a strong
    # association between age and county missingness.
    ...
    # recipients that did not fill in these questions for multiple survey
    # attempts. They may have been uncomfortable sharing that information.
    ...
    # There do not appear to be recipients that have changed their stage.
    # Let's check for recipients that have changed from Not Active to Active
    # accounts
    ...
    # Also the Start group is bigger with 148 recipients compared
    # to only 38 in the Review group.
    # the Start group success rate only needs to be 38/148 ~ 26% to match
    # a 100% success rate in the
    # Review group. It would be good to check this with historical data on the
    # conversion success rate.
    # Data on the cost of converting the Start group vs the Review group would
    # also be helpful
    # for the decision.
    merged.stage.value_counts(dropna=False)
    ...
    print(odds_ratios['OR'][1] - 1)
    # Each additional increase of one year in age is associated with a 18
    # percent decrease in odds of having at least one successful survey.

    # It appears that older recipients are less likely to respond to surveys.
    # Focusing on the older recipients would help target those with low
    # response rates.

    # 3.2
    # We include other factors in our logistic regression to check for
    # confounders.
    big_logit = smf.logit(
        'one_success ~ time_county + age + month',
        data=merged
    ).fit()

    print(big_logit.summary())
    # when we include time spent in the county and the month of the survey,
    # no predictor is signficant

    another_logit = smf.logit(
        'one_success ~ account_status',
        data=merged
    ).fit()
    print(another_logit)
    ...
    print("Best score using built-in LogisticRegCV: %f"
    ...
    # It appears that the account_status and age have the largest effect
    # on whether a survey is successful.
    # Age has a negative effect however. Recall that account_status and age
    # are correlated.
    merged.groupby('account_status')['age'].mean()
    # The Not active accounts are slightly older.
    # Run a test for the equality of means. The null hypothesis is that the
    # means are equal.
    ...
    # The p-value is close to zero, rejecting the null hypothesis that the
    ...
    print(f"Mean accuracy on test data is "
          f"{reg_extra_tree.score(X_test, y_test)}")
    if show_plots:
        ...
        plt.show()
        plt.clf()
    # This time the model has age as the most important effect. All the
    # effects are chosen by the model, however. Note that there is randomness
    # in the model, so sometimes some features will be ranked differently.
    # But the model will still usually choose all of the features for
    # prediction.
